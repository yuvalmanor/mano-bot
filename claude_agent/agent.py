...
TOOL_PERMISSIONS: dict[str, str] = {
    ...
    "notion_save_recipe": "recipes",
    "notion_list_recipes": "recipes",
    "notion_get_recipe": "recipes",
    "fetch_url": "web",
...
async def _dispatch_tool(name: str, args: dict, user_phone: str | None = None) -> str:
    ...
    if name == "notion_get_knowledge":
        ...
        return "error"
    if name == "notion_save_recipe":
        return await notion.add_recipe(
            title=args["title"],
            tags=args.get("tags"),
            content=args.get("content"),
            source_url=args.get("source_url"),
        )
    if name == "notion_list_recipes":
        text = await notion.list_recipes(tag=args.get("tag"))
        return text or "(no recipes)"
    if name == "notion_get_recipe":
        status, content = await notion.get_recipe(title=args["title"])
        if status == "ok":
            return content
        if status == "not_found":
            return "not_found"
