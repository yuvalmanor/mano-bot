...
| 3 | Claude integration | ✅ Done | 🔴 | Code + 10 mocked tests passing; all 4 TESTING.md rows live-verified 2026-05-17. Mid-verification behavior change: system prompt now mirrors user's language per message instead of Hebrew-default (commit 0f95196). |
...
| 5 | Notion integration | 🟨 Code done, live verification pending | 🔴 | **NEXT.** Adapter aligned with real schema (Task/Idea titles + Bucket relation to My Life Buckets); 28 Notion tests (136 total) passing. Idea Lab now covers add (with bucket) / list / archive / comment, plus 5-min dedupe on add_task+add_idea and a per-turn write-tool guard in `claude_agent/agent.py`. Live smoke-tested: add + bucket + comment flow work end-to-end; remaining live-verification rows + Eden-denial still pending. Known open bug: [ISSUE-001](KNOWN_ISSUES.md) — stray "already added" tail message after the comment flow ends with "no". |
...
| 6b | Gmail integration — live OAuth + verification | 🟨 Tokens live, end-to-end folded into 6c | 🟡 | OAuth flow run 3× (personal/cgm/deals) via new `--manual` mode (port-binding-free); 3× `GOOGLE_TOKEN_*` set in Railway. Live send verification rolls into Task 6c (which adds read + contacts and re-OAuths the same accounts with broader scopes). |
| 6c | Gmail read + contacts lookup + trash + default-personal routing | ✅ Done | 🔴 | New `integrations/contacts.py` (People API: searchContacts + otherContacts:search); `gmail.search_inbox` (all 3 accounts, Primary-tab default w/ Workspace fallback); `gmail.trash_by_query`; new tools `gmail_search_inbox`, `gmail_trash_email`, `contacts_lookup`; system prompt rules for "send from cgm to <person>" multi-turn flow, default=personal, trash flow, read across all accounts; OAuth helper scopes expanded (`gmail.readonly`, `gmail.modify`, `contacts.readonly`, `contacts.other.readonly`); em-dash scrubber + Primary-tab scoping; 161 tests passing. Two re-OAuth rounds done, all 3 token env vars updated. Live-verified end-to-end (send, read, default-personal, contacts, trash, Workspace fallback for deals). |
| 6d | Eden Gmail wiring (her personal cgm) | 🟨 Code done, OAuth + Railway env pending | 🔴 | Per-user `google_tokens` map in `users.py`, optional `GOOGLE_TOKEN_EDEN_CGM` in config, `user_phone` plumbed through gmail/contacts/drive — Eden's `cgm` resolves to her own token, Yuval's `cgm` unchanged. Eden has `gmail` permission. 171 tests pass (10 new in `tests/test_user_account_resolution.py`). Pending: Eden runs `scripts/oauth_setup_google.py --manual` against `edeng.cgm@gmail.com`, paste base64 token into Railway as `GOOGLE_TOKEN_EDEN_CGM`, live-verify she can send/read on her cgm via WhatsApp. |
...
| 7b | Google Calendar — live verification | ✅ Done | 🔴 | All flows live-verified: create (relative date, default 1h + 10-min alert), custom alert, no alert, list upcoming, cancel by name, ambiguous cancel. |
...
| 8b | Google Drive — live verification | 🔲 Not started | 🔴 | Unblocked by Task 2b SIM. Run after Task 7b. |
