...
| 3 | Claude integration | 🔄 Live verification in progress | 🔴 | **NEXT.** Code + 10 mocked tests passing; one Hebrew exchange verified during Task 2b. Remaining live rows in TESTING.md: English-input → Hebrew reply, "reply in English" switch, tone/conciseness. Run before Task 5. |
...
| 5 | Notion integration | 🟨 Code done, live verification pending | 🔴 | Adapter aligned with real schema (Task/Idea titles + Bucket relation to My Life Buckets); 18 Notion tests (65 total) passing; Mano Bot connected to Headquarters + Idea Lab parent pages; 4 NOTION_* env vars set in Railway. SIM now available — run TESTING.md Task 5 rows (add/list/filter/idea/Eden-denial) right after Task 3. |
