...
| 6c | Gmail read + contacts lookup + default-personal routing | 🟨 Code done, live verify pending | 🔴 | New `integrations/contacts.py` (People API: searchContacts + otherContacts:search); `gmail.search_inbox` (cgm-only); new tools `gmail_search_inbox`, `contacts_lookup`; system prompt rules for "send from cgm to <person>" multi-turn flow, default=personal, cgm-only read; OAuth helper scopes expanded (`gmail.readonly`, `contacts.readonly`, `contacts.other.readonly`); 10 new tests (146 total) passing. Requires Yuval re-OAuth all three accounts. |
